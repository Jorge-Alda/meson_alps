...
def match(couplings: ALPcouplings, two_loops = False) -> ALPcouplings:
    ...
    kFCNC = (np.einsum('im,nj,mn->ijm', Vckm.H, Vckm, couplings['kU'])[:,:,2] + np.einsum('im,nj,mn->ijn', Vckm.H, Vckm, couplings['kU'])[:,:,2]) * (-0.25*np.log(couplings.scale**2/mtop**2)-3/8+0.75*gx)
    kFCNC += Vtop*couplings['kU'][2,2]
    kFCNC += Vtop*couplings['ku'][2,2]*(0.5*np.log(couplings.scale**2/mtop**2)-0.25-1.5*gx)
    ...
    values |= {'kD': couplings['kD'] + 3/(8*np.pi**2)*Delta_kF['D']*np.eye(3) + kFCNC}
