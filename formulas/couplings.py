...
def cgamma_eff_caPhi(ma):
    ...
    return -0.25*alpha_em(ma)/np.pi*(-(1-light_quarks)*ma**2/(mpi_pm**2-ma**2)+B0)
...
def cgamma_eff_cW(ma):
    ...
    x = 4*mW**2/ma**2
    return s2w + 2*alpha_em(mW)/np.pi*B2(x)
...
def cll_eff_cW(m_ferm, f_a=1000):
    return alpha_em(m_ferm)/np.pi*(9/4*np.log(f_a/mW)/s2w + 6*s2w*np.log(mW/m_ferm))

@lru_cache
def cll_eff_cB(m_ferm, f_a=1000):
    return alpha_em(m_ferm)/np.pi*(15/4*np.log(f_a/mW)/(1-s2w) + 6*(1-s2w)*np.log(mW/m_ferm))
